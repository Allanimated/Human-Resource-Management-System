...
const EmployeeViewTrainings = () => {
  ...
  const handleDeleteTraining = (trainingId) => {
    fetch(`/delete_single_employee_trainings/${trainingId}`, {
      ...
      .then((res) => {
        ...
        const updatedTrainings = fetchedTrainings.filter(
  ...
  return (
    ...
    >
      ...
      {!loading &&
        fetchedTrainings.length === 0 &&
        newlyAppliedTrainings.length === 0 && (
          <h3>Manager has not recommended any trainings for you.</h3>
        )}
            ...
            <tbody>
              {fetchedTrainings.map((training, index) => (
                <tr key={index}>
                  ...
                  {/* No action for fetched trainings */}
                </tr>
              ))}
