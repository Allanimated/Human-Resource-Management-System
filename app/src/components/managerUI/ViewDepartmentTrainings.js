...
function ViewDepartmentTrainings() {
  ...
  return (
    ...
    >
      <h1
        style={{ marginLeft: "370px", marginBottom: "20px", marginTop: "20px" }}
      >
        Training for my Department Staff{" "}
      </h1>
      {error && <p>{error}</p>}
      <table
            ...
            <tr key={employee.id}>
              ...
              <td>
                <ul>
                  {employee.assigned_trainings.map((training) => (
                    <li key={training.id}>
                      {training.id} {training.title} - {training.start_date} to{" "}
                      {training.end_date}, {training.start_time} to{" "}
                      {training.end_time}
                    </li>
                  ))}
                </ul>
              ...
              <td>
                <button
                  ...
                  onClick={() => handleRecommend(employee.id)}
                >
